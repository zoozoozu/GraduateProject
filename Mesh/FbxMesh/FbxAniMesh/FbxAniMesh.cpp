...
CFbxAnimMesh::CFbxAnimMesh(ID3D11Device *pd3dDevice, char *pszFileName, float fSize)
	: CFbxMesh(pd3dDevice)
{
	...
	UINT pnBufferStrides[5] = { sizeof(XMFLOAT3), sizeof(XMFLOAT3), sizeof(XMFLOAT2), sizeof(D3DXVECTOR4), sizeof(D3DXVECTOR4) };
...
void CFbxAnimMesh::UpdateBoneTransform(ID3D11DeviceContext *pd3dDeviceContext, int nAnimationNum, int nNowFrame)
{
	for (int i = 0; i < m_nBoneCount; i++)
	{
...
void CFbxAnimMesh::MakeBoneMatrix(int nNowframe, int nAnimationNum, int nBoneNum, D3DXMATRIX& BoneMatrix)
{
...
void CFbxAnimMesh::CreateConstantBuffer(ID3D11Device *pd3dDevice)
{
...
bool CFbxAnimMesh::FBXFrameAdvance(float fTimeElapsed)
{
...
void CFbxAnimMesh::SetAnimation(int nFBXAnimation)
{
