...
float4 PSSkinned(VS_SKINNED_OUTPUT input) : SV_Target
{
    ...
    input.positionW = normalize(input.positionW);
    ...
    float fRimLightColor = smoothstep(1.0f - rimWidth, 1.0f, 1 - saturate(abs(dot(input.normalW, vCameraPos))));
    cColor.r += (fRimLightColor * 0.5f);
